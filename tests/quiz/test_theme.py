from app.old_quiz.models import Theme, ThemeModel
...
class TestThemeAddView:
    ...
    async def test_missing_title(self, authed_cli):
        resp = await authed_cli.post("/store_flora.add_theme", json={})
    ...
    async def test_different_method(self, authed_cli):
        resp = await authed_cli.get("/store_flora.add_theme", json={})
...
class TestThemeList:
    async def test_unauthorized(self, cli):
        resp = await cli.get("/store_flora.list_themes")
        assert resp.status == 401
        data = await resp.json()
        assert data["status"] == "unauthorized"

    async def test_empty(self, authed_cli):
        resp = await authed_cli.get("/store_flora.list_themes")
        assert resp.status == 200
        data = await resp.json()
    ...
    async def test_different_method(self, authed_cli):
        resp = await authed_cli.post("/store_flora.list_themes")
        assert resp.status == 405
        data = await resp.json()
        assert data["status"] == "not_implemented"
