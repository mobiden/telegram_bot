...
class ReplyKeyboardBottomClass:
    ...
    request_contact: Optional[bool] = field(
        default=False
    )  # направит контакты пользователя
...
class InlineKeyboardBottomClass:
    ...
    url: Optional[str] = field(
        default=""
    )  # URL, который откроется при нажатии на кнопку
    callback_data: Optional[str] = field(
        default=""
    )  # Данные, которые будут отправлены в callback_query при нажатии на кнопку
    switch_inline_query: Optional[str] = field(
        default=""
    )  # Если этот параметр задан, то при нажатии на кнопку приложение предложит пользователю выбрать любой чат, откроет его и вставит
    ...
    switch_inline_query_current_chat: Optional[str] = field(
        default=""
    )  # Если установлено, нажатие кнопки вставит имя пользователя бота и указанный встроенный запрос в поле ввода текущего чата.
...
class ReplyMarkupClass:
    ...
    selective: Optional[bool] = field(
        default=False
    )  # Этот параметр нужен, чтобы показывать клавиатуру только определённым пользователям. Цели: 1) пользователи,
