...
from app.store.telegram_api.te_dataclasses import (
    InlineKeyboardBottomClass,
    OutMessage,
    InlineMarkupClass,
    ReplyKeyboardBottomClass,
    ReplyMarkupClass,
)
from app.store.telegram_api.te_schemes import (
...
class InlineBottom:
    ...
    async def create(bott_list: Optional[list] = None) -> dict:
        ...
        for bottom in bott_list:
            ib = InlineKeyboardBottomClass(
                text=str(bottom[0]), callback_data=str(bottom[1])
            )
...
class ReplyKeyboard:
    ...
    async def create(bott_list: list) -> dict:
        ...
        mb = ReplyMarkupClass(
            keyboard=[reply_keyboard], resize_keyboard=True, one_time_keyboard=True
        )
...
async def create_message(
    chat_id: str, text: str, markup_json: Optional[dict] = {}
) -> OutMessage:
